...
from quarkchain.local import GetBlockTemplateRequest, NewTransaction
    ...
    async def startMining(self):
        ...
        if resp.isRootBlock:
            ...
            if self.miningBlock is not None and not self.isMiningBlockRoot and \
                ...
                # Fill transactions
                block.txList.extend(self.utxoPool[block.header.branch.getShardId()])
                ...
                if submitResp.resultCode == 0:
                    ...
                    if not self.isMiningBlockRoot:
                        ...
                        for oldTx in block.txList[1:]:
                            ...
                            tx.sign([self.genesisId.getKey()])
                            newTxList.append(tx)
                        self.utxoPool[block.header.branch.getShardId()] = newTxList
