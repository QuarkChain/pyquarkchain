...
async def run_master(
    ...
    **kwargs
    ...
    cmd = (
        "pypy3 master.py --cluster_config={} --db_path_root={} "
        ...
        "--devp2p_additional_bootstraps={}".format(
            ...
            kwargs["devp2p_ip"],
            kwargs["devp2p_port"],
            kwargs["devp2p_bootstrap_host"],
            kwargs["devp2p_bootstrap_port"],
            kwargs["devp2p_min_peers"],
            kwargs["devp2p_max_peers"],
...
async def run_slave(
    port, id, shard_mask_list, db_path_root, clean, enable_transaction_history
):
...
class Cluster:
    ...
    async def run_master(self):
        master = await run_master(
            ...
            seed_port=self.config["master"]["seed_port"],
            mine=self.mine,
            clean=self.clean,
            ...
            devp2p_additional_bootstraps=self.config["master"][
    ...
    async def run_slaves(self):
        for slave in self.config["slaves"]:
            s = await run_slave(
                port=slave["port"],
                id=slave["id"],
                shard_mask_list=slave["shard_masks"],
                db_path_root=slave["db_path_root"],
                clean=self.clean,
...
def main():
    ...
    parser.add_argument("--p2p_port", default=DEFAULT_ENV.config.P2P_SERVER_PORT)
    parser.add_argument("--json_rpc_port", default=38391, type=int)
    parser.add_argument("--json_rpc_private_port", default=38491, type=int)
    parser.add_argument(
        "--enable_transaction_history",
        action="store_true",
        default=False,
    ...
    parser.add_argument("--seed_host", default=DEFAULT_ENV.config.P2P_SEED_HOST)
    parser.add_argument("--seed_port", default=DEFAULT_ENV.config.P2P_SEED_PORT)
    ...
    cluster = Cluster(
        config, filename, args.mine, args.clean, args.enable_transaction_history
    )
