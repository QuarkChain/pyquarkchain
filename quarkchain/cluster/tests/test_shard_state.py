...
def create_default_shard_state(env, shard_id=0, diff_calc=None):
    ...
    shard_state = ShardState(env=env, full_shard_id=shard_id, diff_calc=diff_calc)
...
class TestShardState(unittest.TestCase):
    def setUp(self):
        ...
        self.shard_coinbase = config.SHARDS[0].COINBASE_AMOUNT
    ...
    def test_shard_state_difficulty(self):
        env = get_test_env()
        for shard in env.quark_chain_config.SHARDS:
    ...
    def test_shard_state_recovery_from_root_block(self):
        ...
        recoveredState = ShardState(env=env, full_shard_id=0)
