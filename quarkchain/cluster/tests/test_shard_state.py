...
class TestShardState(unittest.TestCase):
    ...
    def testOneTx(self):
        ...
        block, i = state.getTransactionByHash(tx.get_hash())
        ...
        resp = state.getTransactionReceipt(tx.get_hash())
    ...
    def testDuplicatedTx(self):
        ...
        self.assertTrue(state.db.containTransactionHash(tx.get_hash()))
        self.assertFalse(state.add_tx(tx))
    ...
    def testTwoTxInOneBlock(self):
        ...
        block, i = state.getTransactionByHash(b1.txList[0].get_hash())
        self.assertEqual(block, b1)
        self.assertEqual(i, 0)

        block, i = state.getTransactionByHash(b1.txList[1].get_hash())
        self.assertEqual(block, b1)
    ...
    def testXshardTxReceived(self):
        ...
        b1 = state1.getTip().create_block_to_append()
        tx = create_transfer_transaction(
            shardState=state1,
            key=id1.get_key(),
            fromAddress=acc2,
    ...
    def testXshardForTwoRootBlocks(self):
        ...
        b2 = state0.getTip().create_block_to_append()
        state0.finalizeAndAddBlock(b2)
    ...
    def testForkResolve(self):
        ...
        b0 = state.getTip().create_block_to_append()
        b1 = state.getTip().create_block_to_append()
    ...
    def testRootChainFirstConsensus(self):
        ...
        b0 = state0.getTip().create_block_to_append(address=acc1)
        b2 = state0.getTip().create_block_to_append(address=Address.create_empty_account())
    ...
    def testShardStateForkResolveWithHigherRootChain(self):
        ...
        b1 = state.getTip().create_block_to_append()
        b2 = state.getTip().create_block_to_append(nonce=1)
        b2.header.hashPrevRootBlock = rB.header.get_hash()
        b3 = state.getTip().create_block_to_append(nonce=2)
    ...
    def testShardStateRecoveryFromRootBlock(self):
        ...
        blockMetas = []
        for i in range(12):
            b = state.getTip().create_block_to_append(address=acc1)
        ...
        state.finalizeAndAddBlock(b1)
        self.assertEqual(state.db.getMinorBlockByHash(b1.header.get_hash()), b1)
        ...
        # forks are pruned
        self.assertIsNone(recoveredState.db.getMinorBlockByHash(b1.header.get_hash()))
        self.assertEqual(recoveredState.db.getMinorBlockByHash(b1.header.get_hash(), consistencyCheck=False), b1)
    ...
    def testNotUpdateTipOnRootFork(self):
        ...
        m1 = state.getTip().create_block_to_append(address=acc1)
        ...
        # m2 is added
        self.assertEqual(state.db.getMinorBlockByHash(m2.header.get_hash()), m2)
    ...
    def testAddRootBlockRevertHeaderTip(self):
        ...
        m2 = state.getTip().create_block_to_append(address=acc1)
        state.finalizeAndAddBlock(m2)
