...
class TestCluster(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_primary_account_data(self):
        ...
        async with ClusterContext(1, acc1) as clusters:
            ...
            self.assertEqual(
                (await master.get_primary_account_data(acc1)).transaction_count, 1
            )
            self.assertEqual(
                (await master.get_primary_account_data(acc2)).transaction_count, 0
            )
    ...
    async def test_add_transaction(self):
        ...
        async with ClusterContext(2, acc1, should_set_gas_price_limit=True) as clusters:
            ...
            self.assertFalse((await master.add_transaction(tx0)))
    ...
    async def test_add_transaction_with_invalid_mnt(self):
        ...
        async with ClusterContext(2, acc1, should_set_gas_price_limit=True) as clusters:
            ...
            self.assertFalse((await master.add_transaction(tx1)))
            ...
            self.assertFalse((await master.add_transaction(tx2)))
    ...
    async def test_broadcast_cross_shard_transactions(self):
        ...
        async with ClusterContext(1, acc1) as clusters:
            ...
            self.assertTrue(
                (await master.add_raw_minor_block(b1.header.branch, b1.serialize()))
            )
            self.assertTrue(
                (await master.add_raw_minor_block(b2.header.branch, b2.serialize()))
            )
            self.assertTrue(
                (await master.add_raw_minor_block(b3.header.branch, b3.serialize()))
            )
            self.assertTrue(
                (await master.add_raw_minor_block(b4.header.branch, b4.serialize()))
            )
            self.assertEqual(
                (await 
    ...
    async def test_broadcast_cross_shard_transactions_1x2(self):
        ...
        async with ClusterContext(1, acc1, chain_size=8, shard_size=1) as clusters:
            ...
            self.assertTrue(
                (await master.add_raw_minor_block(b5.header.branch, b5.serialize()))
            )
            self.assertEqual(
                (await 
    ...
    async def test_broadcast_cross_shard_transactions_2x1(self):
        ...
        async with ClusterContext(
            ...
            self.assertTrue(
                (await master.add_raw_minor_block(b6.header.branch, b6.serialize()))
            )
            balances = [
                120 * 10 ** 18  # root block coinbase reward
                + 1500000  # root block tax reward (3 blocks) from minor blocks
