...
class Peer(P2PConnection):
    ...
    def __init__(self, env, reader, writer, network, master_server, cluster_peer_id, name=None):
        if name is None:
    ...
    async def start(self, is_server=False):
        ...
        self.master_server.handle_new_root_block_header(self.best_root_block_header_observed, self)
        return None
    ...
    async def handle_get_root_block_header_list_request(self, request):
        ...
        for i in range(request.limit):
            header = self.root_state.db.get_root_block_header_by_hash(block_hash, consistency_check=False)
    ...
    async def handle_get_root_block_list_request(self, request):
        ...
        for h in request.root_block_hash_list:
            r_block = self.root_state.db.get_root_block_by_hash(h, consistency_check=False)
            if r_block is None:
                continue
...
class SimpleNetwork:
    ...
    async def connect(self, ip, port):
        ...
        try:
            ...
            return None
        peer = Peer(self.env, reader, writer, self, self.master_server, self.__get_next_cluster_peer_id())
        peer.send_hello()
    ...
    async def connect_seed(self, ip, port):
        ...
        for peer_info in resp.peer_info_list:
            asyncio.ensure_future(self.connect(
                str(ipaddress.ip_address(peer_info.ip)), peer_info.port))
