...
ROOT_BLOCK_TIME = 60  # actual ~ 57.79
ROOT_EPOCH_INTERVAL = 525600
ROOT_BLOCK_REWARD = 156000000000000000000

SHARD_BLOCK_TIME = 10  # actual ~ 9.76
SHARD_EPOCH_INTERVAL = 3153600
...
def get_circulating_supply(ts, with_new_shards=False):
    ...
    bns = [(ts - GENESIS_TIMESTAMP) // ROOT_BLOCK_TIME]
    bns += [(ts - GENESIS_TIMESTAMP) // SHARD_BLOCK_TIME] * 8
    if with_new_shards:
        ...
        sbreward = SHARD_BLOCK_REWARD
        for i in range((ts - GENESIS_TIMESTAMP) // SHARD_BLOCK_TIME // SHARD_EPOCH_INTERVAL):
            bns.append((ts - GENESIS_TIMESTAMP) // SHARD_BLOCK_TIME - SHARD_EPOCH_INTERVAL * (i + 1))
    return get_circulating_supply_by_bn(bns)


def print_end_of_month_supply():
    import calendar
    import datetime

    dt = datetime.datetime(2021, 12, 31, 0, 0)
    print("{}: {}".format(dt, get_circulating_supply(int(dt.timestamp()), with_new_shards=True) / 1e18))
...
print(get_circulating_supply(40000000000, with_new_shards=False) / 1e18) # 8049840000.0 (max cap without new shards)
print(get_circulating_supply(40000000000, with_new_shards=True) / 1e18) # 9928860000.0 (max cap)
