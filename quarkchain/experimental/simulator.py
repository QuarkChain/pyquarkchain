...
class Node:
    ...
    async def mineOneChain(self):
        ...
        self.mineFuture = asyncio.ensure_future(asyncio.sleep(mineTime))
        self.mineChainId = bestChainId
        self.mineEco = maxEco
        try:
            await self.mineFuture
        except asyncio.CancelledError:
            return

        bestBlock.header.nTime = time.time()
        if bestChainId == 0:
...
def main():
    ...
    for i in range(NODE_SIZE):
        if i < NODE_POWERFUL_MINER_SIZE:
            ...
            node = Node(chainSelector=FixChainSelector(i))
        else:
            node = Node()
